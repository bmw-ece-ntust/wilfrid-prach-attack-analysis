...
```
```math
E[X] = Y(N_{S,i}, N_{C,i}, O) = p \times ( 1 + Y(N_{S,i} - p, N_{C,i}, O-1) ) + (1 - p) \times ( 0 + Y(N_{S,i}, N_{C,i} - (1 - p), O-1) )
```
```math
Y(N_{S,i}, N_{C,i}, O) =
\begin{cases} 
...
p \times ( 1 + Y(N_{S,i} - p, N_{C,i}, O-1) ) + (1 - p) \times Y(N_{S,i}, N_{C,i} - (1 - p), O-1), & \text{otherwise}
\end{cases}
```
-->
